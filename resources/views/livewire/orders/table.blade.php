    ...
    <div class="overflow-x-auto">
        <table>
            ...
            <tbody class="text-sm divide-y divide-gray-100">
                @foreach ($orders as $order)
                    <tr>
                        <td>
                            <div class="text-left">{{ $order->nro }}</div>
                        </td>
                        <td>
                      ...
                      {{--   <td>
                        ...
                        <td>
                            <div>
                                <x-secondary-button wire:click="$emit('open-form', {{$order->id}})">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                </x-secondary-button>
                                <x-secondary-button wire:click="deleteOrders({{ $order }})">
        ...
        @if($orderSelected)
            <x-confirmation-modal wire:model="confirmDeletionIsOpen">
                ...
                <x-slot name="content">
                    ¿Estas seguro de eliminar la orden <span class="font-bold">{{ $orderSelected['nro'] }}</span>?
                </x-slot>
                <x-slot name="footer">
                    <x-secondary-button wire:click="$toggle('deleteOrders')" wire:loading.attr="disabled">
                    Cancelar
                    </x-secondary-button>
                    <x-danger-button class="ml-2" wire:click="deleteOrders({{ $orderSelected['id'] }})" wire:loading.attr="disabled">
                    Eliminar
                    </x-danger-button>
                </x-slot>
